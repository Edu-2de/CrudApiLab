    ...
    <div className="relative w-screen min-h-[60vh] md:min-h-[70vh] flex items-center justify-center px-0 py-0 mt-24 md:mt-1">
      <div className="relative h-[70vh] w-full max-w-[96%] mx-auto overflow-hidden flex gap-4 p-4 transition-all duration-700 ease-in-out">
        {categories.map((cat, idx) => (
          <div
            key={cat.title}
            className={`group rounded-lg shadow p-6 flex flex-col items-center min-h-[200px] min-w-[200px] justify-center hover:rounded-[46%] transition-all duration-900 ease-in-out select-none cursor-pointer ${
          ...
          >
            ...
            <div className="flex flex-col gap-2 w-[90vw] max-h-0 overflow-hidden group-hover:max-h-40 transition-all duration-700 ease-in-out">
              {Object.values(cat.options).map((opt, i) => (
                <button
                  key={i}
                  className="relative left-[40%] rounded-4xl cursor-pointer text-gray-800 px-4 py-2 border-2 border-transparent opacity-0 group-hover:opacity-100 group-hover:text-white group-hover:border-white hover:bg-white hover:text-gray-800 transition-all duration-700 ease-in-out max-w-[20%]"
                >
                  {opt}
                </button>
              ))}
