    ...
    <div className="relative w-screen min-h-[60vh] md:min-h-[70vh] flex items-center justify-center px-0 py-0 mt-24 md:mt-1">
      <div className="relative h-[70vh] w-full max-w-[95%] mx-auto overflow-hidden flex gap-6  p-8">
        {categories.map((cat, idx) => (
          <div
            key={cat.title}
            className={`rounded-lg shadow p-6 flex flex-col items-center min-w-[200px] justify-between hover:rounded-full transition-rounded transition-colors duration-300 ease-in-out ${cardColors[idx % cardColors.length]}`}
          >
